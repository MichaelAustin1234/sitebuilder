/**
 * BFF Route Handler — POST /api/dashboard/site/banner
 *
 * Proxy multipart/form-data upload ke Laravel endpoint
 * POST /api/v1/dashboard/site/banner, menggunakan auth_token dari httpOnly cookie.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const laravelRes = await fetch(`${API_URL}/api/v1/dashboard/site/banner`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      { success: false, message: "Server error saat upload banner", data: null },
      { status: 500 }
    );
  }
}
